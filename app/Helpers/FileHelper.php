    ...
    public static function uploadFile($file, $folder, $id = null, $class = null, $custom = null, $type = null)
    {
        ...
        if ($file->isValid()) {
            ...
            if (!is_dir($destinationPath)) {
                FileSystem::makeDirectory($destinationPath, 755, true);
            }
            // Resize ảnh nếu là ảnh bài viết, sản phẩm, dịch vụ
            // Type = 1 =>> ảnh giáo viên
...
//                ->save($destinationPath . DIRECTORY_SEPARATOR . $destinationFile);
                ...
                Image::make($file)->resize(1000, 650)->save($destinationPath . DIRECTORY_SEPARATOR . $destinationFile);
            } else if ($type == 2) {
...
//                Image::make($file)->resize(650, 500)->save($destinationPath . DIRECTORY_SEPARATOR . $destinationFile);
            } else if ($type == 5) {
                Image::make($file)->resize(80, 80)->save($destinationPath . DIRECTORY_SEPARATOR . $destinationFile);
            } else if ($type == 6) {
                $file->move($destinationPath, $destinationFile);
