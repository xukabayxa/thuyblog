...
if (!function_exists('getBlockName')) {
...
if (!function_exists('getBlockLink')) {
    function getBlockLink($id) {
        $block = \App\Model\Admin\Block::where('id', $id)->first();
        if(isset($block)) {
            return $block->link;
        }
...
if (!function_exists('getBlockImage')) {
    function getBlockImage($id) {
        $block = \App\Model\Admin\Block::where('id', $id)->first();
        if(isset($block)) {
            $img_path = $block->image->path;
            return $img_path;
        }
        return '';
